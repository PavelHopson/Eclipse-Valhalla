...
export const TTSView: React.FC = () => {
  ...
  const handleSpeak = async () => {
    ...
    try {
      ...
      setError('Voice synthesis failed. Try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="mx-auto flex h-full max-w-3xl flex-col justify-center gap-8">
      <div className="text-center">
        ...
        <div className="text-[10px] uppercase tracking-[0.32em] text-[#7F7A72]">Voice chamber</div>
        <h1 className="mt-3 font-ritual text-3xl text-[#F2F1EE] md:text-4xl">Voice forge</h1>
        <p className="mt-3 text-sm leading-6 text-[#B4B0A7]">
          Turn written commands, scripts, and invocations into a controlled voice output. This should feel like summoning system speech, not pressing a generic TTS button.
        </p>
      </div>

      <div className="rounded-[28px] border border-white/10 bg-[#121212]/96 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.38)]">
        <div className="mb-3 text-[10px] uppercase tracking-[0.22em] text-[#7F7A72]">Text to voice</div>
        <textarea
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder="Write the words the chamber should speak..."
        ...
        <div className="mt-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="text-[11px] uppercase tracking-[0.14em] text-[#7F7A72]">
            {text.length} characters / model: gemini-2.5-flash-preview-tts / voice: Kore
          </div>
          <button
            onClick={handleSpeak}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-[22px] border border-white/8 bg-[#121212]/92 p-5">
          ...
          <div className="mt-3 text-sm leading-6 text-[#B4B0A7]">
            Tight, controlled synthesis for commands, scripts, prompts, and spoken ritual text.
          </div>
        </div>
        <div className="rounded-[22px] border border-white/8 bg-[#121212]/92 p-5">
          <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] text-[#7F7A72]">
          ...
          <div className="mt-3 text-sm leading-6 text-[#B4B0A7]">
            Read battle plans aloud, convert Oracle verdicts into voice, or prototype ritual feedback for focus mode.
          </div>
        </div>
      </div>
    </div>
